...
namespace Microsoft.DotNet.Interactive.PowerShell
{
    ...
    {
        public static PowerShellKernel UseDefaultFormatting(this PowerShellKernel kernel)
        {
            var command = new SubmitCode($@"
using namespace {typeof(PocketView).Namespace};
");
        ...
        public static PowerShellKernel UseKernelHelpers(this PowerShellKernel kernel)
        {
            var command = new SubmitCode($@"
using namespace {typeof(Kernel).Namespace};
");
        ...
        public static PowerShellKernel UseNugetDirective(this PowerShellKernel kernel)
        {
        ...
        internal static KernelCommandInvocation DoNugetRestore(PowerShellKernel kernel, PackageRestoreContext restoreContext)
        {
                        ...
                        foreach (var resolvedReference in result.ResolvedReferences)
                        {
                            context.Publish(
                                new DisplayedValueUpdated(
                                    $"Installed assembly {resolvedReference.PackageName} version {resolvedReference.PackageVersion}",
